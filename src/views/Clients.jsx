...
export default function Clients({ data, setClients, onDelete }) {
  ...
  // FIXED: State management for all fields
  ...
  const handleAddNewClient = (e) => {
    ...
    if (!formData.name || !formData.phone) return alert('Name aur Phone Number zaroori hain!');
  ...
  return (
    <div className="space-y-4">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <form onSubmit={handleAddNewClient} className="w-full max-w-sm bg-[#fdf6e9] p-5 rounded-3xl shadow-2xl space-y-3 max-h-[90vh] overflow-y-auto">
            ...
            <input type="text" placeholder="Full Name" className="w-full p-2.5 rounded-xl border" required onChange={(e) => setFormData({...formData, name: e.target.value})} />
            <input type="tel" placeholder="WhatsApp Number (923...)" className="w-full p-2.5 rounded-xl border" required onChange={(e) => setFormData({...formData, phone: e.target.value})} />
            
            <div className="grid grid-cols-2 gap-2">
              <input type="number" placeholder="Total Suits" className="p-2.5 rounded-xl border" onChange={(e) => setFormData({...formData, suits: e.target.value})} />
              <div className="flex items-center gap-2 px-2">
                <input type="checkbox" id="urgent" className="w-5 h-5" onChange={(e) => setFormData({...formData, isUrgent: e.target.checked})} />
            ...
            <div className="bg-white p-3 rounded-xl border border-dashed border-gray-300 space-y-2">
              ...
              <input type="number" placeholder="Silayi Price" className="w-full p-2 rounded-lg border" onChange={(e) => setFormData({...formData, silayi: e.target.value})} />
      ...
      {/* ... (Keep existing Naap Modal Logic) */}
    </div>
  );
}
